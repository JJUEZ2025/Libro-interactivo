document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Colores usados para meta[name="theme-color"]
    const themeColors = {
        light: '#fdf6e3',
        sepia: '#f4ecd8',
        bone: '#f2f0e9',
    ...
    function applyTheme(themeName) {
        ...
        // Sincronizar meta theme-color (status bar + nav bar Android)
        const metaThemeColor = document.querySelector('meta[name=theme-color]');
        if (metaThemeColor && themeColors[themeName]) {
            metaThemeColor.setAttribute('content', themeColors[themeName]);
        }

        // Si estamos en fullscreen, forzar que el root tenga el mismo color
    ...
    function updateNavigationList() {
        ...
        recentMilestones.forEach((pageId) => {
            ...
            if (pageData.scenes && pageData.scenes.length > 0) {
                const cleanText = pageData.scenes[0].replace(
                    /<[^>]*>?/gm,
                    '',
                );
            ...
            li.innerHTML = `
                <span class="history-title">
                    ${title}
                    <span class="history-tag">Volver aquí</span>
                </span>
        ...
        if (recentMilestones.length === 0) {
            historyList.innerHTML =
                '<li class="history-item"><span class="history-preview">Avanza para ver decisiones.</span></li>';
        }
    }
    ...
    async function loadStory() {
        try {
            ...
            pageWrapper.innerHTML =
                '<div class="page-content"><p>Error al cargar historia.</p></div>';
        }
    }
    ...
    function renderPage(pageId) {
        ...
        if (!pageData) {
            pageWrapper.innerHTML =
                '<div class="page-content"><p>Página no encontrada.</p></div>';
            return;
        }
        ...
        if (pageData.images && pageData.images.length > 0) {
            contentHtml += `<div class="images-container">
                ${pageData.images
                    .map(
                        (url) =>
                            `<img src="${url}" alt="Ilustración" loading="eager">`,
                    )
                    .join('')}
            </div>`;
        }
    ...
    function updateUI() {
        ...
        const progress =
            (uniqueVisitedPages.size / totalPagesInStory) * 100 || 0;
    ...
    function toggleFullscreen() {
        if (!document.fullscreenElement) {
            // Android permite navigationUI: 'hide' (cuando está soportado)
    ...
    function handleFullscreenChange() {
        ...
        if (document.fullscreenElement) {
            ...
            // Forzar el mismo color en html y body
            const bg = getComputedStyle(document.body)
                .getPropertyValue('--bg-color')
                .trim();
    ...
    // En iOS no mostramos botón de fullscreen
